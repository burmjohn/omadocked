...
TestCase {
 ...
 function test_stationary(data) {
  ...
  mouseMove(d,d.rowX+d.slotSize*1.5,d.height-30); wait(250);
  const s=d.renderedSlots[1];
  ...
  const start=JSON.stringify({slot:d.renderedSlots[1],input:d.inputRects,scale:art.scale,owner:d.hoveredIndex});
  for(let n=0;n<15;++n) {d.applications=apps.map((a,i)=>Object.assign({},a,{active:i===n%2}));wait(100);verify(findChild(d,"art-menu")===art);}
  console.log(data.tag,JSON.stringify({x:x,y:y,owners:owners,pointers:pointers,scales:scales,inputs:inputs,tips:tips,owner:d.hoveredIndex,caption:tip.captionId,slot:d.renderedSlots[1],artY:art.y,source:String(art.source),input:d.inputRects}));
  compare(pointers,0);compare(owners,0);compare(scales,0);
  compare(JSON.stringify({slot:d.renderedSlots[1],input:d.inputRects,scale:art.scale,owner:d.hoveredIndex}),start);
  if(d.hoveredIndex===1){compare(d.tooltipIndex,1);compare(tip.captionId,"menu");}
 }
}
