...
TestCase {
    ...
    function test_tileIndependentParkingAndRecoveryControlsUseExactKeys() {
        ...
        dock.openContext(1); here.Accessible.pressAction();
        ...
        dock.openContext(1); origin.Accessible.pressAction();
    ...
    function test_separatorAndDisabledLauncherNeverActivate() {
        ...
        dock.selectIndex(1); dock.selectIndex(2);
        ...
        dock.enterKeyboard(); keyClick(Qt.Key_Right);
        compare(dock.focusIndex, 2, "stable indices retained, separator skipped");
        keyClick(Qt.Key_Left); compare(dock.focusIndex, 0);
        dock.openContext(1);
    ...
    function test_newWindowAndCustomContextUseExactIdentity() {
        ...
        alpha.canNewInstance = false; dock.applications = [alpha]; dock.openContext(1);
        ...
        dock.closeMonitorPicker(); dock.openContext(1);
        ...
        dock.openContext(1); mouseClick(findChild(dock, "context-remove"));
        ...
        custom.canEdit = false; dock.applications = [custom]; dock.openContext(1);
    ...
    function test_managedReorderAndSharedBinding() {
        ...
        mousePress(dock, dock.renderedSlots[1].center, y);
        mouseMove(dock, dock.renderedSlots[3].right, y);
        verify(dock.dragActive);
        mouseRelease(dock, dock.renderedSlots[3].right, y);
        ...
        compare(dock.order, ["menu", "beta", "gamma", "alpha"]);
    ...
    function test_liveIdentityAndStalePress() {
        ...
        let y = dock.rowY + 22;
        mousePress(dock, dock.renderedSlots[1].center, y);
        mouseMove(dock, dock.renderedSlots[2].right, y);
        verify(dock.dragActive);
        dock.applications = [app("gamma", false, true)];
        verify(!dock.dragActive);
        mouseRelease(dock, dock.renderedSlots[1].center, dock.rowY + 22);
    ...
    function test_contextActionsAndKeyboard() {
        ...
        dock.openContext(1);
        mouseClick(dock, dock.renderedSlots[1].center, dock.rowY + 22);
        compare(dock.contextIndex, -1);
    ...
    function test_emptyAndDynamicApplications() {
        failOnWarning(/TypeError/);
        compare(dock.order, ["menu"], "production defaults have no demo apps");
        ...
        compare(dock.order, ["menu", "alpha", "beta"]);
        ...
        verify(dock.renderedSlots[20].right <= dock.width);
        dock.applications = [];
        compare(dock.order, ["menu"]);
